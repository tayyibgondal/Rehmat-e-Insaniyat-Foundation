...
def fire_employee(request, pk):
    ...
    if request.method == "POST":
        messages.success(request, 'Employee fired successfully!')
        employee.delete()
...
def update_benificiary(request, pk):
    ...
    if request.method == 'POST':
        ...
        if form.is_valid():
            messages.success(request, 'Benificiary data updated successfully!')
            form.save()
            return redirect('benificiaries')
    context = {
        'form': form,
...
def remove_benificiary(request, pk):
    ...
    if request.method == "POST":
        messages.success(request, 'Benificiary removed successfully!')
        benificiary.delete()
        return redirect('benificiaries')
...
def terminate_drive(request, pk):
    ...
    if request.method == "POST":
        messages.success(request, 'Drive terminated successfully!')
        drive.delete()
        return redirect('drives')
...
def update_past_drive(request, pk):
    ...
    if request.method == 'POST':
        ...
        if form.is_valid():
            messages.success(request, 'Past drive updated successfully!')
            form.save()
...
def add_ambulance(request):
    ...
    if request.method == 'POST':
        ...
        if(form.is_valid()):
            messages.success(request, 'Ambulance added to records successfully!')
            form.save()
...
def update_ambulance(request, pk):
    ...
    if request.method == 'POST':
        ...
        if form.is_valid():
            messages.success(request, 'Ambulance info updated successfully!')
            form.save()
            return redirect('ambulances')
    context = {
        'form': form,
...
def remove_ambulance(request, pk):
    ...
    if request.method == "POST":
        messages.success(request, 'Ambulance removed successfully!')
        ambulance.delete()
        return redirect('ambulances')
...
def add_item(request):
    ...
    if request.method == 'POST':
        ...
        if(form.is_valid()):
            ...
            remaining = donation.amount - donation.used
            needed = int(request.POST.get('price_per_item')) * int(request.POST.get('item_count'))

            if(remaining < needed):
                context = {
                    'form': form,
                }
                messages.error(request, 'The selected donation has not enough amount left!')
                return render(request, 'employees/model_form.html', context)
...
def deliver_item(request, pk):
    ...
    if request.method == "POST":
        messages.success(request, 'Item removed from records successfully!')
        item.delete()
        return redirect('inventory')

    return render(request, 'employees/delete.html', {'obj': item})
...
def add_med_item(request):
    ...
    if request.method == 'POST':
        ...
        if(form.is_valid()):
            messages.success(request, 'Medical item added successfully!')
            form.save()
...
def add_blood_item(request):
    ...
    if request.method == 'POST':
        ...
        if(form.is_valid()):
            messages.success(request, 'Blood item added successfully!')
            form.save()
...
def deliver_blood_item(request, pk):
    ...
    if request.method == 'POST':
        ...
        if blood_item.count < req_count:
            ...
            messages.success(request, 'Blood delivered successfully!')
            blood_item.delete()
        else:
...
def answer_question(request, pk):
    ...
    if request.method == 'POST':
        ...
        if form.is_valid():
            messages.success(request, 'Answered submission successful!')
            form.save()
...
def remove_faq(request, pk):
    ...
    if request.method == "POST":
        messages.success(request, 'FAQ removed successfully!')
        faq.delete()
        return redirect('faqs')
...
def remove_pitch(request, pk):
    ...
    if request.method == "POST":
        messages.success(request, 'Pitch removed successfully!')
        pitch.delete()
...
def make_vol_available(request, pk):
    ...
    if volunteer.available == True:
        ...
        volunteer.save()
        messages.success(request, 'Volunteer Made Available!')
        return redirect('volunteers')
...
def make_vol_unavailable(request, pk):
    ...
    if volunteer.available == False:
        ...
        volunteer.save()
        messages.success(request, 'Volunteer Freed!')
        return redirect('volunteers')
